...
def build_parser():
    ...
    parser.add_argument(
        "--enable-logging",
        dest="enable_logging",
        help="Enable comprehensive logging for test operations",
        action="store_true",
        default=True,
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if options.mode == "train":
        ...
        # demo for elegantrl with crypto data
        kwargs = (
            {}
        )  # in current meta, with respect ccxt, kwargs is {}. For other data sources, such as joinquant, kwargs is not empty
        train(
            start_date=TRAIN_START_DATE,
            end_date=TRAIN_END_DATE,
            ticker_list=DOW_30_TICKER,
            data_source="custom_yahoofinance",
            time_interval="1D",
        ...
        # demo for elegantrl with custom yahoo finance data
        # in current meta, with respect custom_yahoofinance, kwargs is {}. For other data sources, such as joinquant, kwargs is not empty
        kwargs = {}
